...
环境补全中心新增“实验方案一（6PPD-Q / NAFLD）”板块，单独检查 Vina、GROMACS、gmx_MMPBSA、R CellChat、PoseBusters 和 Python 科学包版本；使用教程新增实验方案一场景与数据隐私说明；结果清单补充 `plan_coverage.json/md`、`environment_audit.json` 和 `RESULTS_SUMMARY.md` 的用途。Vina/GROMACS 版本不一致时会明确显示，不自动伪装为方案指定版本。
